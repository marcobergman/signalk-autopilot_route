...
function getRoutePositionBearing(app, currentPosition, guideRadius, maxErrorAngle, routePoints) {
       //
       //  {
       //    guidePoint: 
       //    guidePointBearing: 
      ...
      for (var r = 0; r < routePoints.length - 1; r++) {
          ...
          intersections.forEach(i => {
              ...
              app.debug ("intersectionBearing", intersectionBearing, "segmentHeading", segmentHeading, "difference", difference);
              if (-90 < difference && difference < +90 && onSegment) {
                  app.debug("guidePoint", r, "guidePointBearing", intersectionBearing);
                  result.guidePoint = i;
          ...
          if (closestDistance === null || previousPointDistance < closestDistance) {
              ...
              app.debug("closestDistance", closestDistance);
          }
          ...
          if (delta.updates) {
			...
			if (!activeRoute) routePoints = [[previousPoint.longitude, previousPoint.latitude], [nextPoint.longitude, nextPoint.latitude]];
		        ...
		        maxErrorAngle = configuration["maxErrorAngle"]
                        result = getRoutePositionBearing(app, currentPosition, guideRadius, maxErrorAngle, routePoints);
                        app.debug(result);
                        var apbXte 
		        ...
		        headingPoint = geolib.computeDestinationPoint(currentPosition, guideRadius, result.headingToSteer);
                      ...
                      if (vp.value) nextPoint = vp.value.position; else nextPoint = null;
                    } else if (vp.path === 'navigation.course.previousPoint') {
                      previousPoint = vp.value.position
